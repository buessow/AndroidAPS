...
import app.aaps.database.transactions.SyncNsHeartRatesTransaction
...
class PersistenceLayerImpl @Inject constructor(
    ...
    override fun getLastHeartRateId() = repository.getLastHeartRateId()
    ...
    override fun getNextSyncElementHeartRate(id: Long): Maybe<Pair<HR, HR?>> =
        repository.getNextSyncElementHeartRate(id)?.let {
    ...
    override fun syncNsHeartRate(heartRates: List<HR>): Single<PersistenceLayer.TransactionResult<HR>> {
        ...
        return repository.runTransactionForResult(
            ...
            PersistenceLayer.TransactionResult<HR>().apply {
                tr.inserted.forEach { l(Action.HEART_RATE, it) }
                tr.invalidated.forEach { l(Action.HEART_RATE_REMOVED, it) }
                inserted.addAll(tr.inserted.map(HeartRate::fromDb))
                updated.addAll(tr.updated.map(HeartRate::fromDb))
                invalidated.addAll(tr.invalidated.map(HeartRate::fromDb))
            }
        }
    }

    override fun updateNsIdHeartRateTransaction(heartRates: List<HR>) =
        repository.runTransactionForResult(UpdateNsIdHeartRateTransaction(
            heartRates.map(HR::toDb))).map { tr ->
                PersistenceLayer.TransactionResult<HR>().apply {
                    updated.addAll(tr.updatedNsId.map(HeartRate::fromDb))
                }
        }
