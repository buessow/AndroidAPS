...
@Suppress("UNCHECKED_CAST") class GraphData(
    ...
    fun addBgReadings(addPredictions: Boolean, context: Context?) {
        ...
        addSeries(overviewData.mlPredictionGraphSeries as PointsWithLabelGraphSeries<DataPointWithLabelInterface>)
    ...
    fun addMlPrediction(useForScale: Boolean, scale: Double) {
        val series = overviewData.mlPredictionGraphSeries as
            PointsWithLabelGraphSeries<DataPointWithLabelInterface>
        val maxMl = series.highestValueY
        if (useForScale) {
            minY = 0.0
            maxY = maxMl
        }
